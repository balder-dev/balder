...
class Test0InnerSetupFeatureRef(Base0EnvtesterClass):
    """
    This testcase executes the basic example but uses inner-feature-references in the setup feature `SetupFeatureII`
    (references setup feature `SetupFeatureI`) and in setup feature `SetupFeatureIV` (references setup feature
    `SetupFeatureIII`). It reduces the basic implementation to only one Scenario and one Setup, while the devices are
    using `FeatureI`/`FeatureII` and `FeatureIII`/`FeatureIV`.
    """
