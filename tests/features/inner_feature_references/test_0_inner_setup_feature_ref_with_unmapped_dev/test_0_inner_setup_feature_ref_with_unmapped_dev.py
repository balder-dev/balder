...
class Test0InnerSetupFeatureRefWithUnmappedDev(Base0EnvtesterClass):
    """
    This testcase executes the basic example but uses inner-feature-references in the setup feature. The features
    refer themselves like shown in the following:

    * setup feature `SetupFeatureII` (references setup feature `SetupFeatureI`)
    * setup feature `SetupFeatureIV` (references setup feature `SetupFeatureIII`).
    * setup feature `SetupFeatureVI` (references setup feature `SetupFeatureV`)
    * setup feature `SetupFeatureVII` (references setup feature `SetupFeatureVII`).
