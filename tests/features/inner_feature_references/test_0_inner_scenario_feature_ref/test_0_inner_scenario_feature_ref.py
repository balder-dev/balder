...
class Test0InnerScenarioFeatureRef(Base0EnvtesterClass):
    """
    This testcase executes the basic example but uses inner-feature-references in the scenario feature `FeatureII`
    (references scenario feature `FeatureI`) and in setup feature `SetupFeatureIV` (references scenario feature
    `FeatureIII`). It reduces the basic implementation to only one Scenario and one Setup, while the devices are
