...
class Test0All(Base0EnvtesterClass):
    """
    ...
    `balder .. --only-with-scenario scenarios/scenario_*.py --only-with-setup setups/setup*.py`

    All elements should be executed here.
    """

    @property
    def cmd_args(self):
        return ["--only-with-scenario", "scenarios/scenario_*.py", "--only-with-setup", "setups/setup*.py"]
