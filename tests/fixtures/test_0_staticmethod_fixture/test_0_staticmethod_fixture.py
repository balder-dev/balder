...
class Test0StaticmethodFixture(Base0EnvtesterClass):
    """
    This testcase executes the envtester environment. This is a reduced version of the environment tester. The test
    checks if fixtures that are defined as static methods (only in scenario and setup class) work as expected.

    For this the following will be checked:

    * fixture will be called correctly
    * parameters are given correctly (no automatic self for the first parameter - type will be checked too)
    * return value of fixture will be forwarded correctly
    ...
    execution levels. It has one setup and one scenario class that use static method fixtures here. The test uses
    the following fixtures to secure that parameter and return values are managed correctly:

    **Class SetupA**:

    * ``fixture_setup_session`` (**non-static**) expects parameters ``balderglob_fixture_session``
    * ``fixture_setup_setup`` (**static**) expects parameters ``balderglob_fixture_setup`` and ``fixture_setup_session``
    * ``fixture_setup_scenario`` (**non-static**) expects parameters ``balderglob_fixture_scenario`` and
      ``fixture_setup_setup``
    * ``fixture_setup_variation`` (**static**) expects parameters ``balderglob_fixture_variation`` and
      ``fixture_scenario``
    * ``fixture_setup_testcase`` (**non-static**) expects parameters ``balderglob_fixture_testcase`` and
      ``fixture_setup_variation``

    **Class ScenarioA**:

    * ``fixture_scenario_session`` (**static**) expects parameters ``balderglob_fixture_session`` and
      ``fixture_setup_session``
    * ``fixture_scenario_setup`` (**non-static**) expects parameters ``balderglob_fixture_setup``,
      ``fixture_setup_setup`` and ``fixture_scenario_session``
    * ``fixture_scenario_scenario`` (**static**) expects parameters ``balderglob_fixture_scenario``,
      ``fixture_setup_scenario`` and ``fixture_scenario_setup``
    * ``fixture_scenario_variation`` (**non-static**) expects parameters ``balderglob_fixture_variation``,
      ``fixture_setup_variation`` and ``fixture_scenario_scenario``
    * ``fixture_scenario_testcase`` (**static**) expects parameters ``balderglob_fixture_testcase``,
