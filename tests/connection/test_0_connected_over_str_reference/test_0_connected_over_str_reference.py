...
def test_0_connected_over_str_reference(balder_working_dir):
    """
    This testcase executes the basic example and checks if the tree ends with the result SUCCESS. This environment uses
    string references for all `@connect(..)` decorator of scenario- and setup-devices. It expects that balder can
    resolve them without problems and works in the same way as using normal referenced devices. The environment
    decorates the devices in another order between ``ScenarioA`` and ``ScenarioB`` or ``SetupA`` and ``SetupB`` too.

    This means, that in..

    * ``ScenarioA``: device ``ScenarioDevice1`` is decorated with the `@connect(..)` decorator (``ScenarioDevice2`` is
      not decorated)
    * ``ScenarioB``: device ``ScenarioDevice4`` is decorated with the `@connect(..)` decorator (``ScenarioDevice3`` is
      not decorated)
    * ``SetupA``: device ``SetupDevice2`` is decorated with the `@connect(..)` decorator (``SetupDevice1`` is not
      decorated)
    * ``SetupB``: device ``SetupDevice3`` is decorated with the `@connect(..)` decorator (``SetupDevice4`` is not
      decorated)

    All of them use string references
    """
