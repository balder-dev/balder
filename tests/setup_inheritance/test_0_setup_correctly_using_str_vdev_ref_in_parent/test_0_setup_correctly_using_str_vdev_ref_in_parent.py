...
class Test0SetupCorrectlyUsingStrVDevRefInParent(Base0EnvtesterClass):
    """
    This testcase executes the basic envtester environment but uses an inherited setup `SetupAChild`. Within this
    setup we define an additional feature class that was not known in parent setup `SetupAParent`. In addition
    to that, the environment overwrites an existing feature within the `SetupAChild`-Device that is a child class of
    the overwritten one.
    Compared to `test_0_setup_correctly_overwrite_and_add_feature`,  this environment uses string references within
    the parent setup `SetupAParent` for assigning a VDev on scenario level.
    ...
    def expected_data(self) -> tuple:
        return (
            ...
            {"cls": "SetupFeatureIOverwritten", "meth": "do_something", "category": "feature"},
            {"cls": "SetupFeatureII", "meth": "do_something", "category": "feature"},
            {"cls": "SetupFeatureIOverwritten", "meth": "call_over_vdev", "category": "feature"},
            {"cls": "SetupFeatureNew", "meth": "do_something", "category": "feature"},
