...
def test_0_no_multiple_variations_for_non_connection(balder_working_dir):
    """
    ...
    has no Connection with any other device. We expect that balder will create only one variation for the
    ``SetupDevice2a`` within the ``ScenarioA`` <=> ``SetupA`` constellation.
