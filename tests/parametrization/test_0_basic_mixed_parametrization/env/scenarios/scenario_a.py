...
class ScenarioA(balder.Scenario):
    ...
    @balder.parametrize('test1', ['a1', 'a2', 'a3', 'a4'])
    @balder.parametrize_by_feature('test2', (ScenarioDevice2, 'ii', 'get_dynamic_parameters'))
    def test_a_1(self, test1, test2):
        RuntimeObserver.add_entry(__file__, ScenarioA, ScenarioA.test_a_1, category="testcase",
                                  msg=f"execute Test `{ScenarioA.test_a_1.__qualname__}` with Parameter "
                                      f"`test1={test1}` and `test2={test2}`")
