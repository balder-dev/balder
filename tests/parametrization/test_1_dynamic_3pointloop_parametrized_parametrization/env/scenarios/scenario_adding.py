...
class ScenarioAdding(balder.Scenario):
    ...
    @balder.parametrize_by_feature('number2', (NumberTwoDevice, 'example', 'get_random_numbers_higher_than'), parameter={'value': Parameter('number3')})
    @balder.parametrize_by_feature('number3', (NumberTwoDevice, 'example', 'get_random_numbers_higher_than'), parameter={'value': Parameter('number1')})
    def test_add_two_numbers(self, number1, number2, number3):
        self.NumberOneDevice.number.set_number(number1 + number3)
        ...
        assert result == number1 + number2 + number3, "result is not as expected"
