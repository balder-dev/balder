...
def test_1_setup_check_no_reduction_of_diff_vdevice_cnns(balder_working_dir):
    """
    ...
    that are used by the ``ScenarioAdding.NumberOneDevice`` and the ``SetupPythonAdd.NumberProvider1``.
    ...
    * ``VDeviceHelperFeature3``: ``MySimplySharedMemoryConnection``
    ...
    The VDevices are mapped on setup level here!
