...
class Test0ScenarioCorrectlyUsingStrVDevRefInParent(Base0EnvtesterClass):
    """
    This testcase executes the basic envtester environment but uses an inherited scenario `ScenarioAChild`. Within this
    scenario we define an additional feature class that was not known in parent scenario `ScenarioAParent`. In addition
    to that, the environment overwrites an existing feature within the `ScenarioAChild`-Device that is a child class of
    the overwritten one.
    Compared to `test_0_scenario_correctly_overwrite_and_add_feature`, this environment uses string references within
    the parent scenario `ScenarioAParent` for assigning a VDev on scenario level.
    ...
    def expected_data(self) -> tuple:
        return (
            ...
            {"cls": "FeatureI", "meth": "call_over_vdev", "category": "feature"},  # because no mapping yet
            ...
            {"cls": "SetupFeatureI", "meth": "do_something", "category": "feature"},
            {"cls": "SetupFeatureII", "meth": "do_something", "category": "feature"},
            {"cls": "SetupFeatureI", "meth": "call_over_vdev", "category": "feature"},
            {"cls": "NewlySetupFeature", "meth": "do_something", "category": "feature"},
